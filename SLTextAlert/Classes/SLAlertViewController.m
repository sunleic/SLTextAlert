...
@property (strong, nonatomic) UITextView *messageLabel;
...
+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message messageAttribute:(NSArray <SLAlertAttributeModel *> *)attributeModels {
    ...
    instance.messageAttributeModels = attributeModels;
    return instance;
}
...
- (void)viewDidLoad {
    ...
    self.messageLabel.attributedText = [self getAttributeStringWithPlainString:self.message];
}
...
- (void)updateMessageLabelFrame {
    ...
    CGFloat messageY = self.title.length ? CGRectGetMaxY(_titleLabel.frame) + 6 : _contentMargin.top;
...
- (UITextView *)creatLabelWithFontSize:(CGFloat)fontSize {
...
- (UITextView *)messageLabel {
    if (!_messageLabel) {
        _messageLabel = [self creatLabelWithFontSize:kMessageFontSize];
        _messageLabel.attributedText = [self getAttributeStringWithPlainString:self.message];
...
- (void)setMessage:(NSString *)message {
    ...
    _messageLabel.attributedText = [self getAttributeStringWithPlainString:message];;
}
...
- (NSMutableAttributedString *)getAttributeStringWithPlainString:(NSString *)message {
    // 设置属性
...
//    NSRange range1 = [_message rangeOfString:@"4008800114"];
//    NSRange range2 = [_message rangeOfString:@"010-82760152"];
//    if (characterRange.location == range1.location) {
//        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"tel://4008800114"] options:@{} completionHandler:^(BOOL success) {
//        }];
//    } else if (characterRange.location == range2.location) {
//        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"tel://010-82760152"] options:@{} completionHandler:^(BOOL success) {
//        }];
//    }
